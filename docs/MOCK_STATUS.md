...
| **Tracking Tuner — ByteTrackConfig** | **MOCK** | ⏳ endpoint ยังไม่มีบน Pi เลย (ไม่ใช่แค่ฮาร์ดแวร์) | `useMockTracking: true` |
...
- **Pi ทำ `/api/tracking/config` เสร็จแล้ว** → ตั้ง `useMockTracking: false`
  ...
  ⚠️ ต้องยืนยันชื่อ field/ค่า default จาก `config.py` จริงก่อน ยังเป็นค่าประมาณการอยู่)
